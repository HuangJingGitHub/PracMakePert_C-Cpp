...
int main(int argc, char** argv) {
    ...
    RRTStarPlanner planner_weight_cost_2(start, end, obs_vec, 15, 20, 10, back_img.size(), 0, 10);
    
    auto start_time = high_resolution_clock::now();
    bool planned_weight_cost = planner_weight_cost_2.Plan(back_img);
    auto end_time = high_resolution_clock::now();
    auto duration_time = duration_cast<milliseconds>(end_time - start_time);
    cout << "Time for planning: " << duration_time.count() << " milliconds\n";
    return 0;    
}
...
void PassageDetectionTest(const int test_num = 100, 
    ...
    for (int test_idx = 0; test_idx < test_num; test_idx++) {
        ...
        auto EV_check_res = ExtendedVisibilityPassageCheck(obs_vec, false, false);
        auto end_time = high_resolution_clock::now();
        auto duration_time = duration_cast<milliseconds>(end_time - start_time);
        EV_check_time[test_idx] = (float) duration_time.count();
        EV_psg_num[test_idx] = EV_check_res.first.size();

        start_time = high_resolution_clock::now();
        vector<vector<int>> cells = ReportGabrielCells(obs_vec, EV_check_res.first);
        ...
        EV_psg_segment_num[test_idx] = EV_check_seg_res.first.size();

        cells = ReportGabrielCells(obs_vec, EV_check_seg_res.first);
        EV_cell_via_segment_num[test_idx] = cells.size();

        // Using Delaunary graph
        start_time = high_resolution_clock::now();
        auto DG_check_res = PassageCheckInDelaunayGraph(obs_vec, false);        
        end_time = high_resolution_clock::now();
        duration_time = duration_cast<milliseconds>(end_time - start_time);

    ...
    while (true) {
        ...
        if (counter == 40) {
            ...
            if (marker_detelected) {
                ...
                if (variance > 0.01) {
                    save_file.open("./src/grasping_position_selection_2d/src/parameters/camera_extrinsic_matrix.txt", std::ios::trunc);
            ...
            if (HSV_thresholds != pre_HSV_thresholds) {
                ...
                save_file.open("./src/grasping_position_selection_2d/src/parameters/HSV_thresholds.txt", std::ios::trunc);
                if (save_file.is_open() == false) {
                    std::cerr << "Can not open the save file!\n"
                                << "File " << __FILE__ << ", "
                                << "Function " << __FUNCTION__ << ", "

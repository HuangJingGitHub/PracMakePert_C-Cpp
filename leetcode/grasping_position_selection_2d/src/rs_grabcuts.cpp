...
int main(int argc, char * argv[]) try
{
    ...
    setMouseCallback("GRB image", onMouse, 0);
    ...
    while (waitKey(1) < 0 && getWindowProperty(window_name, WND_PROP_AUTOSIZE) >= 0)
    {
        ...
        Mat bgModel, fgModel; 
        auto default_rect = Rect(50, 50, 50, 50);
        if (cropRect.width > 0 && cropRect.height > 0) {
            grabCut(color_mat, mask, cropRect, bgModel, fgModel, 1, GC_INIT_WITH_RECT);
        }
        else { 

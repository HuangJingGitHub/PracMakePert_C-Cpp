...
public: 
    ...
    LK_Tracker(const string win_name) {
        ...
        update_rate_ = 0.5;
        pre_Jd_ = Eigen::Matrix<float, points_num_ * 2, 2>::Identity();
    }
    ...
    void UpdateJd() {
        ...
        /*Test start*/
/*         Point2f cur_ee = ee_points_[0][0], pre_ee = ee_points_[1][0];
        Point2f cur_feedback_pt, pre_feedback_pt;
        cur_feedback_pt.x = 2 * cur_ee.x + 2 * cur_ee.y;
        cur_feedback_pt.y = 3 * cur_ee.y + 3 * cur_ee.x;
        pre_feedback_pt.x = 2 * pre_ee.x + 2 * pre_ee.y;
        pre_feedback_pt.y = 3 * pre_ee.y + 3 * pre_ee.x;
        Point2f delta_feedback_pt2f = cur_feedback_pt - pre_feedback_pt,  
                delta_ee_pt2f = cur_ee - pre_ee;
        delta_points(0, 0) = delta_feedback_pt2f.x;
        delta_points(1, 0) = delta_feedback_pt2f.y;
        delta_ee(0, 0) = delta_ee_pt2f.x;
        delta_ee(1, 0) = delta_ee_pt2f.y; */
        /*Test end*/
...
/*             path_accumulated_lengths_ = vector<vector<float>>(path_num_);
    ...
    vector<Point2f> ProjectPtsToPathSet(vector<Point2f> cur_points) {
        ...
        for (int i = 0; i < path_num_; i++) {
            forward_shift_ = path_set_[i].size() / 20;

...
private:
    ...
    Angle3pts angle_obj_;
    float target_angle_value_ = 110;  // deg
...
public:
    ...
    void ProcessImg(const sensor_msgs::ImageConstPtr& msg) {
        ...
        vector<Point2f> cur_feedback_pts = tracker_.GetFeedbackPoints();
        if (!path_set_planned_ 
            && cur_feedback_pts.size() == feedback_pt_num_ 
            && extractor_.obs_extract_succeed_) {
            int pivot_idx = 0;
            float feedback_pts_radius = 10;
            target_feedback_pts_ = cur_feedback_pts;
            vector<float> initial_angle_pos(6, 0), desired_angle_pos,
                          deisred_s_0_pos{312, 185};
                          // deisred_s_0_pos{cur_feedback_pts[0].x - 130, cur_feedback_pts[0].y - 110};
            for (int i = 0; i < feedback_pt_num_; i++) {
                initial_angle_pos[2*i] = cur_feedback_pts[i].x;
                initial_angle_pos[2*i + 1] = cur_feedback_pts[i].y;
            }
            angle_obj_ = Angle3pts(initial_angle_pos);
            angle_obj_.Update(initial_angle_pos);
            feedback_pts_radius = max(angle_obj_.v_1_.norm(), angle_obj_.v_2_.norm());
            feedback_pts_radius = min(feedback_pts_radius, float(25));
            desired_angle_pos = angle_obj_.DetermineTarget(initial_angle_pos, target_angle_value_, deisred_s_0_pos, 0, 0.05,
            ...
            angle_obj_.Update(desired_angle_pos);
            pivot_idx = angle_obj_.GetPivotIndex();
            pivot_idx = 0;
            cout << "Pivot index in target config: " << pivot_idx << "\n"
                 << "pivot position: " << cur_feedback_pts[0] << "\n";

            path_set_ = GeneratePathSet(cur_feedback_pts, target_feedback_pts_, pivot_idx, 
                                        10, extractor_.obs_polygons_, cv_ptr->image);
            path_set_planned_ = !path_set_.empty();
            if (path_set_planned_) {
                path_set_[2] = ProcessCollisionPath(path_set_[0], cur_feedback_pts[0], cur_feedback_pts[2], 
                                                    target_feedback_pts_[2], extractor_.obs_polygons_[0]);
            ...
            tracker_.UpdateJd();
            projection_pts_on_path_set_ = path_set_tracker_.CoordinateProjectPtsToPathSet(tracker_.GetFeedbackPoints());
            extractor_.ProjectDOToObstacles();
            
            vector<float> cur_angle_pos(6, 0);
            for (int i = 0; i < feedback_pt_num_; i++) {
                cur_angle_pos[2*i] = cur_feedback_pts[i].x;
                cur_angle_pos[2*i + 1] = cur_feedback_pts[i].y;
            }
            angle_obj_.Update(cur_angle_pos);
            ...
            for (int i = 0; i < feedback_pt_num_; i++) {
                circle(cv_ptr->image, target_feedback_pts_[i], 3, Scalar(255, 0, 0), -1);
                circle(cv_ptr->image, tracker_.points_[0][i], 3, Scalar(0, 0, 255), -1);
                if (i != 0) {
                    line(cv_ptr->image, target_feedback_pts_[0], target_feedback_pts_[i], Scalar(255, 255, 255), 2);
    ...
    bool GetVisualInfoService(  visual_module::visual_info_service_angle::Request &request,
        ...
        if (tracker_.points_[0].size() < feedback_pt_num_ || tracker_.ee_points_[0].empty() || 
            extractor_.obs_vertices_picked_.size() < extractor_.obs_vertices_num_preset_  ||
            extractor_.DO_extract_succeed_ == false || projection_pts_on_path_set_.size() < feedback_pt_num_)
            return false;
        else if (request.one_time_data_flag) {
            for (int i = 0; i < extractor_.obs_vertices_num_preset_; i++) {
                response.obs_vertices[2*i] = extractor_.obs_vertices_picked_[i].x;
                response.obs_vertices[2*i + 1] = extractor_.obs_vertices_picked_[i].y;
            }
            return true;
        }
        ...
        response.target_angle_value = target_angle_value_;
        response.cur_angle_value = angle_obj_.angle_val_;
        for (int i = 0; i < 6; i++)
            response.angle_gradient[i] = angle_obj_.gradient_(i, 0);

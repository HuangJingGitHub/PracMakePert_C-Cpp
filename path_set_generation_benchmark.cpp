...
int main(int argc, char** argv) {
    ...
    if (path_planned) {
        ...
        DrawPath(back_img, planned_path_pts, cv::viz::Color::blue());
...
/*         for (int i = planner_weight_cost.extended_visibility_passage_pts_.size() + 1; i < add_general_passage_res.second.size(); i++) {
    ...
    if (planned_path_pts.size() > 0) {
        ...
        for (int i = 0; i < init_pts.size(); i++) {
            ...
            RRTStarPlanner cur_planner(init_pts[i], target_pts[i], obs_vec, 15, 20, 10, back_img.size(), 0, 10);
            bool cur_path_planned = cur_planner.PlanHomotopicPath(back_img, planned_path_pts, init_pts[i], target_pts[i]);
            if (cur_path_planned == true) {
                separately_planned_path_set[i] = cur_planner.GetPathInPts();  
                DrawPath(back_img, separately_planned_path_set[i], cv::viz::Color::red()); 
            }
        }
    }

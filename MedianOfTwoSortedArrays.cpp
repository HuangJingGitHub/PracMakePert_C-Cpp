...
class Solution_Palindromic {
public:
	string longestPalindrome(string s) {
		vector<int> indexStore;
		int maxLen = 1, currentLen = 1;
		string currentStr, longestStr;
		for (int i = 1; i < s.size(); i++)
			if (s[i - 1] == s[i] || s[i - 1] == s[i + 1])
				indexStore.push_back(i);
		if (indexStore.size() == 0) {
			longestStr = s.substr(0, 1);
			return longestStr;
		}

		for (int i = 0; i < indexStore.size(); i++) {
			currentStr = palindromeStr(s, indexStore[i]);
			longestStr = (currentStr.size() > longestStr.size()) ? currentStr : longestStr;
		}
		return longestStr;
		...
		for (; (left >= 0) && (right <= s.size() - 1); left--, right++)
		{
			...
			else if (left == 0 || right == s.size() - 1)
			{
				if (left == 0 && right != s.size() - 1)
					temp = (isPalindromic(s.substr(0, right - left + 2))) ? s.substr(left, right - left + 2) : s.substr(left, right - left + 1);
				else if (left != 0 && right == s.size() - 1)
					temp = (isPalindromic(s.substr(left - 1, right))) ? s.substr(left - 1, right - left + 2) : s.substr(left, right - left + 1);
				else
	...
	{
		int len = s.size(), middle, left, right;
		if (len % 2 == 1)
		{
			middle = (len - 1) / 2, left = middle, right = middle;
...
int main()
{
	string testStr = "eabcbae";
	Solution_Palindromic sol;
	cout << sol.longestPalindrome(testStr) << endl;
	cout << sol.isPalindromic("bb");
}
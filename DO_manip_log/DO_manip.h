...
struct angleFeature3Pts{
    ...
    float angle;
    Eigen::Matrix<float, 1, 6> gradient;
};
...
public:
    ...
    void getManipulability(angleFeature3Pts angFeature){
        ...
        for (int i = 0; i < 3; i++){
            cout << "OK3\n";
            cout << "tracked pts: " << angFeature.sPt.size() << endl;
            ...
            if (linel*refl >= 0 && liner*refr >= 0){
                ...
                cout << "OKX1\n";
                Wi[i] = 1 / (distance[i] + episiloni);
            }
            else if (linel*refl < 0){
                EIndictor[i] = 0;
                distance[i] = abs(linel) * divCof;
                Wi[i] = 1 / (distance[i] + episilono);
            }
            else{
                EIndictor[i] = 0;
        ...
        for (int i = 0; i < 3; i++){
            cout << "OK5\n";
            sw += (weightedGradient.col(2*i).norm() + weightedGradient.col(2*i+1).norm()) / weightedGradient.norm()
                  * angFeature.sPt[i];
        }
    }
};